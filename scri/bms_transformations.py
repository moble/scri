...
def _process_transformation_kwargs(input_ell_max, **kwargs):
    ...
    if boost.dtype != float or boost.shape != (3,) or beta >= 1.0:
        raise ValueError(f"Input boost=`{boost}` should be a 3-vector with magnitude strictly less than 1.0")
...
def Lorentz_to_spin_matrix(lorentz):
    ...
    psi = 2 * np.arctan2(np.linalg.norm(lorentz.rotation.components[1:]), lorentz.rotation.components[0])
    if psi == 0:
        rotation_vec_hat = np.array([0, 0, 0])
    else:
        rotation_vec_hat = lorentz.rotation.components[1:] / np.linalg.norm(lorentz.rotation.components[1:])

    chi = 1j * np.arctanh(np.linalg.norm(lorentz.boost))
    if chi == 0:
        boost_vec_hat = np.array([0, 0, 0])
    else:
        boost_vec_hat = lorentz.boost / np.linalg.norm(lorentz.boost)
...
def transformed_grid(frame_rotation, boost_velocity, n_theta, n_phi):
    ...
    for j in range(n_theta):
        for k in range(n_phi):
            ...
            R_j_k[j, k] = Bprm_j_k(*quaternion.as_spherical_coords(rotation_quaternion)) * rotation_quaternion
...
class Lorentz_transformation:
    ...
    def copy(self):
        return Lorentz_transformation(rotation=self.rotation, boost=self.boost, order=self.order, ell_max=self.ell_max)

    def reorder(self, output_order):
        """Reorder a Lorentz transformation."""
        if not ("rotation" in output_order and "boost" in output_order):
...
class BMS_transformation:
    ...
    def reorder(self, output_order):
        ...
        if not ("supertranslation" in output_order and "rotation" in output_order and "boost" in output_order):
            raise ValueError("Not enough transformations")
        ...
        if self.order == normal_order:
            ...
            S_prime = transform_supertranslation(
                ...
                Lorentz_transformation(rotation=self.rotation, ell_max=self.ell_max, order=self.order),
            )

            BMS_normal_order = BMS_transformation(
                rotation=self.rotation,
                boost=self.boost,
            ...
            S_prime = transform_supertranslation(
                ...
                Lorentz_transformation(
                    rotation=self.rotation, boost=self.boost, ell_max=self.ell_max, order=self.order
                ),
            )
            ...
            L_prime = Lorentz_transformation(
                rotation=self.rotation, boost=self.boost, ell_max=self.ell_max, order=self.order
            ).reorder(normal_order)

            BMS_normal_order = BMS_transformation(
                rotation=L_prime.rotation,
                boost=L_prime.boost,
                supertranslation=self.supertranslation,
                ell_max=self.ell_max,
                order=normal_order,
            ...
            S_prime = transform_supertranslation(
                self.supertranslation, Lorentz_transformation(boost=self.boost, ell_max=self.ell_max, order=self.order)
            )
        ...
        if output_order == normal_order:
            ...
            S_prime = transform_supertranslation(
                ...
                Lorentz_transformation(
                    rotation=BMS_normal_order.rotation, ell_max=BMS_normal_order.ell_max, order=BMS_normal_order.order
                ).inverse(),
            )

            BMS_reordered = BMS_transformation(
                rotation=BMS_normal_order.rotation,
            ...
            S_prime = transform_supertranslation(
                BMS_normal_order.supertranslation,
                Lorentz_transformation(boost=L_prime.boost, ell_max=L_prime.ell_max, order=L_prime.order).inverse(),
            )
    ...
    def is_identity(self, rtol=1e-5, atol=1e-8, verbose=False):
        ...
        rotation_is_identity = np.allclose(self.rotation.components, [1, 0, 0, 0], rtol, atol)
        boost_is_identity = np.allclose(self.boost, [0, 0, 0], rtol, atol)
        supertranslation_is_identity = np.allclose(
            self.supertranslation, np.zeros_like(self.supertranslation), rtol, atol
        )
        if verbose:
            print(f"rotation is identity: {rotation_is_identity}")
            print(f"boost is identity: {boost_is_identity}")
    ...
    def inverse(self, output_order=None):
        ...
        L_inverse = Lorentz_transformation(
            rotation=bms_normal_order.rotation,
            boost=bms_normal_order.boost,
            ell_max=bms_normal_order.ell_max,
            output_order=bms_normal_order.order,
        ).inverse(output_order=["rotation", "boost"][::-1])
        ...
        bms_inverse = BMS_transformation(
            rotation=L_inverse.rotation,
    ...
    def compose(self, other, output_order=["supertranslation", "rotation", "boost"]):
        ...
        L1 = Lorentz_transformation(
            rotation=bms1_normal_order.rotation,
            boost=bms1_normal_order.boost,
            ell_max=ell_max,
            order=bms1_normal_order.order,
        )
        L2 = Lorentz_transformation(
            rotation=bms2_normal_order.rotation,
        ...
        bms_composed = BMS_transformation(
            rotation=L_composed.rotation,
