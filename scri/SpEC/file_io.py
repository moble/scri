...
def experimental_save(w, file_name=None, L2norm_fractional_tolerance=1e-10, log_frame=None, compress=True):
    import tempfile
    import contextlib
    import h5py
    import scri
    ...
    with contextlib.ExitStack() as context:
        if file_name is None:
            ...
            file_name = f'{temp_dir}/test.h5'
        else:
            print(f'Saving to "{file_name}"')
        with h5py.File(file_name, 'w') as f:
            f.attrs['sxs_format'] = 'experimental'
            warnings.warn('sxs_format is being set to "experimental"')
            f.create_dataset('time', data=w.t, **compression_options)
            f.create_dataset('modes', data=w.data.view(float), chunks=(w.n_times, 1), **compression_options)
            f['modes'].attrs['ell_min'] = w.ell_min
            f['modes'].attrs['ell_max'] = w.ell_max
            if log_frame.size > 1:
                f.create_dataset('log_frame', data=log_frame, chunks=(w.n_times, 1), **compression_options)

        size = os.stat(file_name).st_size
        print(f'Output file size: {size:_} B')

    return w


def experimental_load(file_name):
    import h5py
    import scri
    ...
    with h5py.File(file_name, 'r') as f:
        ...
        warnings.warn('sxs_format is assumed to be "experimental"')
        assert sxs_format == 'experimental'
    ...
    warnings.warn('experimental_load is assuming `h` data')
    w = WaveformModes(
