...
def com_transformation_to_map_to_superrest_frame(
    ...
    of Gfun should be like:
        def Gfun(Gparams, time, *Gargs):
            ...
            return G
    Here Gparams[0:3] should be the components of boost_velocity, Gparams[3:6]
    should be the components of the spatial translation (l=1 supertranslation),
    and there can be any extra fit parameters desired (but their fitted values
    are ignored). The shape of G that is returned by Gfun should be (n,3) where
    n is the length of the time argument. The Gargs arguments are determined
    from the Gargsfun tuple above from the whole abd object (for example, so
    that Gfun can have access to a timeseries of angular velocity or Bondi mass aspect).
