...
def MT_to_WM(h_mts, sxs_version=False, dataType=scri.h):
    ...
    if not sxs_version:
        ...
        h = sxs.WaveformModes(input_array=np.array(h_mts)[:,LM_index(abs(h_mts.s),-abs(h_mts.s),0):],
                              time=h_mts.t,
                              time_axis=0,
                              modes_axis=1,
                              ell_min=abs(h_mts.s),
...
def compute_bondi_rest_mass_and_conformal_factor(PsiM, ell_max):
    ...
    r_vector = 4 * np.pi * charge_vector_from_aspect(r_vector_modes_on_Grid)

    # These come from Eqs. (14) and (15)
    M_Grid = np.sqrt(P[..., 0] ** 2 - (P[..., 1] ** 2 + P[..., 2] ** 2 + P[..., 3] ** 2))
    
    K_Grid = M_Grid / (np.tensordot(P[..., 0], r_vector[0], axes=0) -
...
def compute_Moreschi_supermomentum(abd, alpha, ell_max):
    ...
    for i in range(2 * ell_max + 1):
        for j in range(2 * ell_max + 1):
            alpha_i_j = alpha[i, j]
            PsiM_Grid_interp[i, j] = CubicSpline(
                PsiM.t, PsiM_Grid[:, i, j]
            )(alpha_i_j)
            
            K_Grid_interp[i, j] = CubicSpline(
                PsiM.t, K_Grid[:, i, j]
            )(alpha_i_j)

    # compute the supertranslation term
    alpha_modes = spinsfast.map2salm(alpha.view(np.ndarray), 0, ell_max)
    D_alpha_modes = sxs.waveforms.memory.𝔇(alpha_modes, ell_max)
    D_alpha_modes_Grid = sf.SWSH_grids.Grid(spinsfast.salm2map(D_alpha_modes.view(np.ndarray), 0, ell_max, 2 * ell_max + 1, 2 * ell_max + 1), spin_weight=0)
...
def compute_alpha_perturbation(PsiM, M_Grid, K_Grid, ell_max):
    ...
    alpha = sxs.waveforms.memory.𝔇inverse(PsiM_plus_M_K3, ell_max)
    
    return spinsfast.salm2map(alpha, 0, ell_max, 2 * ell_max + 1, 2 * ell_max + 1).real

def supertranslation_to_map_to_superrest_frame(abd, target_PsiM=None, N_itr_max=10, rel_err_tol=1e-12, ell_max=12, print_conv=False):
    ...
    while itr < N_itr_max and not rel_err < rel_err_tol:
        ...
        if target_PsiM != None:
            ...
            target_PsiM_Grid_interp = sf.SWSH_grids.Grid(np.zeros((2 * target_PsiM.ell_max + 1, 2 * target_PsiM.ell_max + 1), dtype=float), spin_weight=0)
            for i in range(2 * target_PsiM.ell_max + 1):
                for j in range(2 * target_PsiM.ell_max + 1):
                    alpha_i_j = prev_alpha_Grid[i, j]
                    target_PsiM_Grid_interp[i, j] = CubicSpline(
                        target_PsiM.t, target_PsiM_Grid[:, i, j]
                    )(alpha_i_j)
            M_Grid = -target_PsiM_Grid_interp.view(np.ndarray)

        alpha_Grid += compute_alpha_perturbation(PsiM, M_Grid, K_Grid, ell_max)
        
        rel_err = (spinsfast.map2salm((abs(sf.SWSH_grids.Grid(alpha_Grid.copy(), spin_weight=0) - prev_alpha_Grid) /\
                                       abs(sf.SWSH_grids.Grid(alpha_Grid.copy(), spin_weight=0))).view(np.ndarray), 0, ell_max)[LM_index(0, 0, 0)] / np.sqrt(4 * np.pi)).real
...
def com_transformation_to_map_to_superrest_frame(abd, N_itr_max=10, rel_err_tol=1e-12, ell_max=None, space_translation=True, boost_velocity=True, print_conv=False):
...
def rotation_from_spin_charge(chi, t):
    ...
    """
    chi_f = quaterion.quaternion(chi*[np.argmin(abs(t))]).normalized()
...
def rotation_to_map_to_superrest_frame(abd, target_h=None, N_itr_max=10, rel_err_tol=[1e-12, 1e-5], ell_max=None, print_conv=False):
    ...
    if target_h == None:
        ...
        while itr < N_itr_max and not (rel_err < rel_err_tol[0] or rel_err_omega < rel_err_tol[1]):
            ...
            rel_err_omega = simpson(np.linalg.norm(omega - target_omega, axis=1), news.t) /\
                            simpson(np.linalg.norm(target_omega, axis=1), news.t)
...
def map_to_superrest_frame(self, t_0=0,
                           target_h_input=None,\
   ...
   rel_err_tols : dict, optional
    ...
    # space_translation
    space_translation, space_rel_errs = com_transformation_to_map_to_superrest_frame(abd_interp,\
    ...
    alpha, alpha_rel_errs = supertranslation_to_map_to_superrest_frame(abd_prime,\
                                                                        target_PsiM,\
    ...
    rotation, rot_rel_errs = rotation_to_map_to_superrest_frame(abd_prime,\
                                                                 target_h_interp,\
    ...
    CoM_transformation, CoM_rel_errs = com_transformation_to_map_to_superrest_frame(abd_prime,\
                                                                                     N_itr_max=N_itr_maxes['com_transformation'],\
                                                                                     rel_err_tol=rel_err_tols['com_transformation'],\
                                                                                     ell_max=ell_max,\
                                                                                     space_translation=True,\
    ...
    # transform abd
    abd_prime = abd.transform(supertranslation=alpha[:LM_index(alpha_ell_max, alpha_ell_max, 0) + 1],\
                              frame_rotation=rotation)
